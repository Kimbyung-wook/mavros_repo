...
int main(int argc, char **argv)
{
    ...
    uint16_t wp_size = 3;
    uint16_t wp_idx = 0;
    geometry_msgs::Point wp_list[wp_size];
    wp_list[0].x = 5.0;    wp_list[0].y = 5.0;    wp_list[0].z = 2.0;
    wp_list[1].x = 5.0;    wp_list[1].y = 5.0;    wp_list[1].z = 5.0;
    wp_list[2].x = 0.0;    wp_list[2].y = 0.0;    wp_list[2].z = 2.0;
    ...
    ROS_INFO("Start Loop");
    while(ros::ok()){
        double range = RangeBetweenPoseStamped(msg_pose,current_local_pos);
        // Show states
        ...
        case Transitions::arm: //****************************************
            ...
            if( current_state.mode == "OFFBOARD" && current_state.armed){
                ...
                if(range < RangeThreshold){
                    if(ros::Time::now() - last_request > ros::Duration(10.0)){
                        ...
                        ROS_INFO("[INFO] pass to waypoints");
                        last_request = ros::Time::now();
                    }
                }
                // far to the target
            ...
            break;
        case Transitions::Go:
            msg_pose.pose.position = wp_list[wp_idx];
            range = RangeBetweenPoseStamped(msg_pose,current_local_pos);
            // Close to the target
            if(range < RangeThreshold){
                if(ros::Time::now() - last_request > ros::Duration(5.0)){
                    ROS_INFO("[INFO] Change wps");
                    last_request = ros::Time::now();
                    wp_idx++;
                    if(wp_idx >= wp_size){
                        ROS_INFO("[INFO] End wp flight");
